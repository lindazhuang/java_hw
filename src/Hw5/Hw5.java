...
class Q5_1{
	...
	System.out.println(stringBuffer.replace(stringBuffer.indexOf("p"),stringBuffer.indexOf("n")+1, "apple"));
	StringBuilder stringBuilder = new StringBuilder("I have a pen.");
	System.out.println(stringBuilder.replace(stringBuilder.indexOf("p"),stringBuilder.indexOf("n")+1, "apple"));
	}
...
class Q5_2{
	public void date20180401(){
	    ...
	    System.out.println(year1+"年"+month1+"月"+day1+"日到"+year+"年"+month+"月"+day+"日差"+daysBetween(now,c)+"天");
	}
	...
	public void calendar2017() {
		...
		for (int b = 1; b <= 12; b++) {
			...
			for (day_of_month = 1; day_of_month <= day; day_of_month++) {
				...
				if (cal.get(Calendar.DAY_OF_WEEK) == 7) {//String.format("%2d",i) 兩位數控一格
					System.out.println();
			}
